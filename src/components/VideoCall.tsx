...
function CallUI({ channelName, appId }: VideoCallProps) {
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Audio volume indicator already enabled");
    }
  ...
  if (screenTrack) {
    ...
    // Also publish camera when screen sharing for recording
    if (localCameraTrack) publishTracks.push(localCameraTrack);
  } else if (localCameraTrack) {
    // When not screen sharing, publish camera
  ...
  async function stopRecording() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || `Recording stop API returned ${res.status}`);
      }
